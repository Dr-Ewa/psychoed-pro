...
const MEM_TEMPLATE = `Selected subtests of the Wide Range Assessment of Memory and Learning Third Edition (WRAML 3) were administered to examine [firstName]'s auditory and verbal and visual memory for new information, and to determine how well [pronoun] can retain and retrieve material. The Wide Range Assessment of Memory and Learning Third Edition is an individually administered, standardized assessment of memory, learning, and cognitive functions that support memory and learning processes. Specifically, the WRAML 3 provides information about [firstName]'s verbal and visual immediate and delayed recall, recognition, attention and concentration, and working memory.

The General Immediate Memory Index is an estimate of overall immediate recall, measured across a wide range of tasks. The General Immediate Memory Index is derived from the scores earned on the Verbal Immediate Memory Index, the Visual Immediate Memory Index, and the Attention and Concentration Index. [firstName] scored within the [GENERAL_IMMEDIATE_MEMORY_RANGE] range (at around the [GENERAL_IMMEDIATE_MEMORY_PERCENTILE] percentile).

The Visual Immediate Memory Index is an estimate of how well [firstName] can learn and recall both meaningful and minimally related rote visual information. The Visual Immediate Memory Index is derived from the scaled scores earned on Picture Memory and Design Learning. [firstName] demonstrated [VISUAL_IMMEDIATE_MEMORY_RANGE] visual memory skills (at around the [VISUAL_IMMEDIATE_MEMORY_PERCENTILE] percentile).

On the Design Memory subtest, [firstName] was shown a single Design Learning Stimulus Card across four trials to parallel the administration procedures of the Verbal Learning subtest. The card has 18 different geometric shapes distributed across four quadrants. It is exposed for 10 seconds and then removed. After a 10 second delay, [pronoun] was asked to recall and draw the shapes in the correct locations. This procedure was repeated three times for a total of four learning trials. [firstName]'s visual memory skills for simple information are [DESIGN_MEMORY_IMMEDIATE_RANGE] (at around the [DESIGN_MEMORY_IMMEDIATE_PERCENTILE] percentile).

When asked to recall, that is draw, the design details in the correct locations after a 20 to 30 minute delay, [firstName] demonstrated [DESIGN_MEMORY_DELAYED_RANGE] abilities to recall this information from [possessive] long term memory (at around the [DESIGN_MEMORY_DELAYED_PERCENTILE] percentile).

On the Picture Memory subtest, [firstName] was shown a meaningful scene. [pronoun] was then asked to look at a second, similar scene. Memory of the original picture is indicated by identifying elements that were altered in the second picture. [firstName]'s performance was in the [PICTURE_MEMORY_IMMEDIATE_RANGE] range (at around the [PICTURE_MEMORY_IMMEDIATE_PERCENTILE] percentile).

When asked to identify elements that were changed, added, or moved after a 20 to 30 minute delay, [firstName] demonstrated [PICTURE_MEMORY_DELAYED_RANGE] skills (at around the [PICTURE_MEMORY_DELAYED_PERCENTILE] percentile).

The Verbal Immediate Memory Index is an estimate of how well [firstName] can learn and recall both contextually meaningful and relatively less meaningful verbal information. The Verbal Immediate Memory Index is derived from the scaled scores earned on Story Memory and Verbal Learning. [firstName]'s verbal memory skills are [VERBAL_IMMEDIATE_MEMORY_RANGE] (at around the [VERBAL_IMMEDIATE_MEMORY_PERCENTILE] percentile). These subtests base heavily on one's ability to attend to rote visual and verbal information.

Story Memory assesses the student's ability to process, encode, and recall meaningful material that is presented in a sequential format. In the immediate portion, the examiner reads two stories, one at a time, and the examinee is asked to retell each story from memory. [firstName] achieved [STORY_MEMORY_IMMEDIATE_RANGE] scores in immediate memory (at around the [STORY_MEMORY_IMMEDIATE_PERCENTILE] percentile). [pronoun] was able to remember the major points in the sequence or gist of the story.

On a delayed recall format, [firstName] was able to remember information that [pronoun] retained in the immediate portion, putting [possessive] score in the [STORY_MEMORY_DELAYED_RANGE] range (at around the [STORY_MEMORY_DELAYED_PERCENTILE] percentile). [pronoun] experiences difficulties when asked to recognize some details from the story.

[firstName]'s performance on the Verbal Learning subtest, which assesses ability to learn a list of unrelated words over four learning trials, is [VERBAL_LEARNING_IMMEDIATE_RANGE] (at around the [VERBAL_LEARNING_IMMEDIATE_PERCENTILE] percentile). It was noted that [possessive] performance was gradually increasing with each repetition and [firstName] was able to retrieve this information from [possessive] long term memory, putting [possessive] performance in the [VERBAL_LEARNING_DELAYED_RANGE] range (at around the [VERBAL_LEARNING_DELAYED_PERCENTILE] percentile) on the delayed recall section.

The Attention and Concentration Index provides an estimate of how well [firstName] can learn and recall attentionally demanding, relatively rote, sequential information. Both auditory and visual information are sampled. The Attention and Concentration Index is derived from the scaled scores earned on Finger Windows and Number Letter. [firstName]'s Attention and Concentration abilities are [ATTENTION_CONCENTRATION_RANGE] (at around the [ATTENTION_CONCENTRATION_PERCENTILE] percentile).

In the Attention and Concentration domain, on Finger Windows, [firstName]'s ability to recall placement fell in the [FINGER_WINDOWS_RANGE] range (at around the [FINGER_WINDOWS_PERCENTILE] percentile). The Finger Windows subtest assesses the examinee's ability to attend to and remember a sequence of spatial locations using a card with holes, or windows. The pattern of windows becomes progressively longer as the subtest proceeds.

On Number Letter, [firstName] was asked to repeat a random mix of verbally presented numbers and letters. [pronoun] had to hear the items and produce them. [firstName]'s verbal memory skill to produce a set of heard numbers and letters was in the [NUMBER_LETTER_RANGE] range (at around the [NUMBER_LETTER_PERCENTILE] percentile).

The General Delayed Index is an estimate of longer term storage of the information [firstName] learned on the four immediate memory subtests, that is Picture Memory, Story Memory, Design Learning, and Verbal Learning, and is derived from the scores earned on the Visual Delayed and Verbal Delayed Indexes.

The Visual Delayed Index is an estimate of how well [firstName] can retain and retrieve both meaningful and minimally related visual information after a 20 to 30 minute delay. The Visual Delayed Index is derived from the subtest scaled scores of Picture Memory Delayed and Design Learning Delayed.

The Verbal Delayed Index is an estimate of how well [firstName] can store and retrieve both meaningful and minimally related verbal information after a 20 to 30 minute delay. The Verbal Delayed Index is derived from the subtest scaled scores of Story Memory Delayed and Verbal Learning Delayed. [firstName]'s Verbal Delayed memory skills are [VERBAL_DELAYED_MEMORY_RANGE] (at around the [VERBAL_DELAYED_MEMORY_PERCENTILE] percentile).`;
...
To assess [firstName]'s oral language skills, one subtest was administered. [firstName]'s performance in Listening Comprehension was in the [LISTENING_COMPREHENSION_RANGE] range (at around the [LISTENING_COMPREHENSION_PERCENTILE] percentile). This subtest measures a student's ability to understand spoken language and receptive language.

Receptive Vocabulary: Measures listening vocabulary. [firstName] scored on this subtest within the [RECEPTIVE_VOCABULARY_RANGE] range (at around the [RECEPTIVE_VOCABULARY_PERCENTILE] percentile).

Oral Discourse Comprehension: Measures ability to understand spoken passages. [firstName] scored within the [ORAL_DISCOURSE_RANGE] range (at around the [ORAL_DISCOURSE_PERCENTILE] percentile).

The Word Reading subtest measures word recognition accuracy. [firstName] scored in the [WORD_READING_RANGE] range (at around the [WORD_READING_PERCENTILE] percentile).

The Pseudoword Decoding subtest measures ability to decode unfamiliar words. [firstName] scored in the [PSEUDOWORD_DECODING_RANGE] range (at around the [PSEUDOWORD_DECODING_PERCENTILE] percentile).

Reading Comprehension measures ability to understand written passages. [firstName] scored in the [READING_COMPREHENSION_RANGE] range (at around the [READING_COMPREHENSION_PERCENTILE] percentile).

Spelling measures ability to spell dictated words. [firstName] scored in the [SPELLING_RANGE] range (at around the [SPELLING_PERCENTILE] percentile).

Sentence Composition measures ability to write grammatically correct sentences. [firstName] scored in the [SENTENCE_COMPOSITION_RANGE] range (at around the [SENTENCE_COMPOSITION_PERCENTILE] percentile).

Essay Composition measures written expression and organization. [firstName] scored in the [ESSAY_COMPOSITION_RANGE] range (at around the [ESSAY_COMPOSITION_PERCENTILE] percentile).

Math Problem Solving measures mathematical reasoning and applied math skills. [firstName] scored in the [MATH_PROBLEM_SOLVING_RANGE] range (at around the [MATH_PROBLEM_SOLVING_PERCENTILE] percentile).
...
The Basic Reading Composite measures fundamental reading skills including word recognition and decoding. [firstName]'s Basic Reading Composite fell in the [BASIC_READING_RANGE] range (at around the [BASIC_READING_PERCENTILE] percentile).

The Written Expression Composite provides an overall measure of written language skills. [firstName]'s Written Expression Composite fell in the [WRITTEN_EXPRESSION_RANGE] range (at around the [WRITTEN_EXPRESSION_PERCENTILE] percentile).

The Mathematics Composite provides an overall measure of mathematical ability. [firstName]'s Mathematics Composite fell in the [MATHEMATICS_COMPOSITE_RANGE] range (at around the [MATHEMATICS_COMPOSITE_PERCENTILE] percentile).`;
...
Step 2: For each placeholder like [GENERAL_IMMEDIATE_MEMORY_RANGE], determine classification: 40-69=Very Low, 70-79=Low, 80-89=Low Average, 90-109=Average, 110-119=High Average, 120-129=Very High, 130+=Extremely High.
For scaled scores: 1-3=Very Low, 4-5=Low, 6-7=Low Average, 8-12=Average, 13-15=High Average, 16+=Very High.
...
Liam was also administered subtests that contribute to the Cognitive Proficiency Index (CPI). These four subtests are drawn from the working memory and processing speed domains. Liam's index score suggests that he demonstrates somewhat lower than average efficiency when processing cognitive information in the service of learning, problem solving, and higher-order reasoning (at around the 16th percentile). Low CPI scores may occur for many reasons, including visual or auditory processing deficits, inattention, distractibility, visuomotor difficulties, limited working memory storage or mental manipulation capacity, or generally low cognitive ability. The CPI is most informative when interpreted as part of a comprehensive evaluation, together with its counterpart, the GAI. The practitioner may consider evaluating the GAI-CPI pairwise comparison, as this may provide additional interpretive information regarding the possible impact of cognitive processing on his ability. Liam's performance on subtests contributing to the GAI was stronger than his overall level of cognitive proficiency. The difference between his GAI (102) and CPI (85) scores suggests that higher-order cognitive abilities may be a strength compared to abilities that facilitate cognitive processing efficiency. Relative weaknesses in mental control and speed of visual scanning may sometimes create challenges as Liam engages in more complex cognitive processes, such as learning new material or applying logical thinking skills.`,
  ...
  memory: `Selected subtests of the Wide Range Assessment of Memory and Learning Third Edition (WRAML 3) were administered to examine Liam's auditory and verbal and visual memory for new information, and to determine how well he can retain and retrieve material.

The General Immediate Memory Index is an estimate of overall immediate recall. Liam scored within the Average range (at around the 42nd percentile), indicating that his immediate memory functioning is appropriately developed relative to same-aged peers.

The Visual Immediate Memory Index is an estimate of how well Liam can learn and recall visual information. Liam demonstrated Average visual memory skills (at around the 50th percentile).

On the Design Memory subtest, Liam's visual memory skills for simple information are Average (at around the 37th percentile). On the Picture Memory subtest, his ability to recognise previously presented visual details was Average (at around the 55th percentile).

The Verbal Immediate Memory Index is an estimate of how well Liam can learn and recall verbal information. Liam demonstrated Low Average verbal memory skills (at around the 18th percentile). On the Story Memory subtest, his ability to recall meaningful verbal information was Low Average (at around the 21st percentile). On the Verbal Learning subtest, his ability to learn a list of unrelated words across multiple trials was Low Average (at around the 16th percentile).

The Attention and Concentration Index provides an estimate of how well Liam can learn and recall attentionally demanding sequential information. Liam's Attention and Concentration abilities are Low Average (at around the 18th percentile). On Finger Windows, his ability to recall placement fell in the Average range (at around the 37th percentile). On Number Letter, his verbal memory skill to produce a set of heard numbers and letters was in the Low Average range (at around the 12th percentile).
...
To assess Liam's oral language skills, one subtest was administered. Liam's performance in Listening Comprehension was in the Average range (at around the 50th percentile). This subtest measures a student's ability to understand spoken language and receptive language.

Receptive Vocabulary: Measures listening vocabulary. Liam scored on this subtest within the Average range (at around the 55th percentile).

Oral Discourse Comprehension: Measures ability to understand spoken passages. Liam scored within the Average range (at around the 45th percentile).

The Word Reading subtest measures word recognition accuracy. Liam scored in the Low Average range (at around the 16th percentile).

The Pseudoword Decoding subtest measures ability to decode unfamiliar words. Liam scored in the Low range (at around the 7th percentile).

Reading Comprehension measures ability to understand written passages. Liam scored in the Low Average range (at around the 18th percentile).

Spelling measures ability to spell dictated words. Liam scored in the Low Average range (at around the 14th percentile).

Sentence Composition measures ability to write grammatically correct sentences. Liam scored in the Low Average range (at around the 21st percentile).

Essay Composition measures written expression and organization. Liam scored in the Low Average range (at around the 16th percentile).
...
The Total Reading Composite provides an overall measure of reading ability, combining word reading accuracy, decoding, and reading comprehension. Liam's Total Reading Composite fell in the Low Average range (at around the 14th percentile).

The Basic Reading Composite measures fundamental reading skills including word recognition and decoding. Liam's Basic Reading Composite fell in the Low range (at around the 9th percentile).

The Written Expression Composite provides an overall measure of written language skills. Liam's Written Expression Composite fell in the Low Average range (at around the 16th percentile).

The Mathematics Composite provides an overall measure of mathematical ability. Liam's Mathematics Composite fell in the Average range (at around the 40th percentile).
